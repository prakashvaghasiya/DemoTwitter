...
protocol DelegateCheckBox {
    func selectedCheckBox(checkBoxTag: Bool, tag: Int)
}

@IBDesignable
open class CheckBox: UIControl {
    var delegaet : DelegateCheckBox?
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        self.delegaet?.selectedCheckBox(checkBoxTag: isChecked, tag: self.tag)
    }
    
    open override func draw(_ rect: CGRect) {
        
        //Draw the outlined component
        let newRect = rect.insetBy(dx: 1.5 / 2, dy: 1.5 / 2)
